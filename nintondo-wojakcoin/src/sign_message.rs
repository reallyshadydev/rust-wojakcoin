...
/// The prefix for signed messages using pepecoin's message signing protocol.
pub const BITCOIN_SIGNED_MSG_PREFIX: &[u8] = b"\x1aWojakCoin Signed Message:\n";
...
mod message_signing {
    ...
    /// An error used for dealing with pepecoin Signed Messages.
    #[cfg_attr(docsrs, doc(cfg(feature = "secp-recovery")))]
    ...
    /// A signature on a pepecoin Signed Message.
    ///
...
/// Hash message for signature using pepecoin's message signing format.
...
mod tests {
    ...
    fn test_message_signature() {
        ...
        let p2pkh = Address::p2pkh(&pubkey, Network::Wojakcoin);
        assert_eq!(signature2.is_signed_by_address(&secp, &p2pkh, msg_hash), Ok(true));
        let p2wpkh = Address::p2wpkh(&pubkey, Network::Wojakcoin).unwrap();
        assert_eq!(
            signature2.is_signed_by_address(&secp, &p2wpkh, msg_hash),
            Err(MessageSignatureError::UnsupportedAddressType(AddressType::P2wpkh))
        );
        let p2shwpkh = Address::p2shwpkh(&pubkey, Network::Wojakcoin).unwrap();
        assert_eq!(
