...
//! Wojakcoin addresses.
//!
//! Support for ordinary base58 Wojakcoin addresses and private keys.
//!
//! # Example: creating a new address from a randomly-generated key pair
//!
//! ```rust
//! # #[cfg(feature = "rand-std")] {
//! use nintondo_wojakcoin::{Address, PublicKey, Network};
...
//! let address = Address::p2pkh(&public_key, Network::Wojakcoin);
//! # }
//! ```
//!
...
//!
//! ```toml
//! nintondo-wojakcoin = { version = "...", features = ["rand-std"] }
//! ```
...
pub enum Error {
    ...
    /// pepecoin script opcode does not match any known witness version, the script is malformed.
...
impl From<WitnessVersion> for opcodes::All {
    /// Converts [`WitnessVersion`] instance into corresponding pepecoin scriptopcode (`OP_0`..`OP_16`).
...
/// ```rust
/// use std::str::FromStr;
/// use nintondo_wojakcoin::{Address, Network};
/// use nintondo_wojakcoin::address::{NetworkUnchecked, NetworkChecked};
///
/// // variant 1
/// let address: Address<NetworkUnchecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse().unwrap();
/// let address: Address<NetworkChecked> = address.require_network(Network::Wojakcoin).unwrap();
///
/// // variant 2
/// let address: Address = Address::from_str("32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf").unwrap()
///                .require_network(Network::Wojakcoin).unwrap();
///
/// // variant 3
/// let address: Address<NetworkChecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse::<Address<_>>()
///                .unwrap().require_network(Network::Wojakcoin).unwrap();
/// ```
///
...
#[cfg(feature = "serde")]
crate::serde_utils::serde_string_serialize_impl!(Address, "a pepecoin address");

#[cfg(feature = "serde")]
crate::serde_utils::serde_string_deserialize_impl!(Address<NetworkUnchecked>, "a pepecoin address");
...
impl Address {
    ...
    /// Checks whether or not the address is following pepecoin standardness rules when
    ...
    /// Checks whether or not the address is following pepecoin standardness rules.
    ///
    /// SegWit addresses with unassigned witness versions or non-standard program sizes are
    /// considered non-standard.
    ...
    /// # let address = ADDRESS.parse::<nintondo_wojakcoin::Address<_>>().unwrap().assume_checked();
    ...
    pub fn to_qr_uri(&self) -> String {
        let schema = match self.payload {
            Payload::WitnessProgram { .. } => "WOJAKCOIN",
            _ => "wojakcoin",
        };
        format!("{}:{:#}", schema, self)
    }
...
impl Address<NetworkUnchecked> {
    ...
    /// ```rust
    /// use nintondo_wojakcoin::{Address, Network};
    /// use nintondo_wojakcoin::address::NetworkUnchecked;
    ///
    ...
    ///
    /// assert_eq!(address.is_valid_for_network(Network::Wojakcoin), false);
    ///
    /// let address: Address<NetworkUnchecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse().unwrap();
    /// assert!(address.is_valid_for_network(Network::Wojakcoin));
    ...
    pub fn is_valid_for_network(&self, network: Network) -> bool {
        ...
        match (self.network, network) {
            (a, b) if a == b => true,
            (Network::Wojakcoin, _) | (_, Network::Wojakcoin) => false,
...
mod tests {
    ...
    use crate::network::constants::Network::{Wojakcoin, Testnet};
    ...
    #[test]
    fn test_p2wpkh() {
        // stolen from pepecoin transaction: b3c8c2b6cfc335abbcb2c7823a8453f55d64b2b5125a9a61e8737230cdb8ce20
        let mut key = "033bc8c83c52df5712229a2f72206d90192366c36428cb0c12b6af98324d97bfbc"
            .parse::<PublicKey>()
            .unwrap();
        let addr = Address::p2wpkh(&key, Wojakcoin).unwrap();
        ...
        assert_eq!(Address::p2wpkh(&key, Wojakcoin), Err(Error::UncompressedPubkey));
    }

    #[test]
    fn test_p2wsh() {
        // stolen from pepecoin transaction 5df912fda4becb1c29e928bec8d64d93e9ba8efa9b5b405bd683c86fd2c65667
        ...
        let addr = Address::p2wsh(&script, Wojakcoin);
        assert_eq!(
            &addr.to_string(),
    ...
    #[test]
    fn test_p2shwpkh() {
        // stolen from pepecoin transaction: ad3fd9c6b52e752ba21425435ff3dd361d6ac271531fc1d2144843a9f550ad01
        let mut key = "026c468be64d22761c30cd2f12cbc7de255d592d7904b1bab07236897cc4c2e766"
            .parse::<PublicKey>()
            .unwrap();
        let addr = Address::p2shwpkh(&key, Wojakcoin).unwrap();
    ...
    #[test]
    fn test_p2shwsh() {
        // stolen from pepecoin transaction f9ee2be4df05041d0e0a35d7caa3157495ca4f93b233234c9967b6901dacf7a9
        ...
        let addr = Address::p2shwsh(&script, Wojakcoin);
    ...
    fn test_non_existent_segwit_version() {
        ...
        let addr = Address::new(Wojakcoin, Payload::WitnessProgram(witness_prog));
        roundtrips(&addr);
    }
    ...
    fn test_address_type() {
        ...
        for (address, expected_type) in &addresses {
            let addr = Address::from_str(address)
                .unwrap()
                .require_network(Network::Wojakcoin)
                .expect("mainnet");
    ...
    fn test_qr_string() {
            ...
            ["132F25rTsvBdp9JzLLBHP5mvGY66i1xdiM", "33iFwdLuRpW1uK1RTRqsoi8rR4NpDzk66k"].iter()
        {
            ...
            assert_eq!(addr.to_qr_uri(), format!("wojakcoin:{}", el));
        }
        ...
        .iter()
        {
            ...
            assert_eq!(addr.to_qr_uri(), format!("WOJAKCOIN:{}", el.to_ascii_uppercase()));
        }
    }
    ...
    fn test_valid_networks() {
        ...
        const LEGACY_EQUIVALENCE_CLASSES: &[&[Network]] =
            &[&[Network::Wojakcoin], &[Network::Testnet, Network::Regtest, Network::Signet]];
        const SEGWIT_EQUIVALENCE_CLASSES: &[&[Network]] =
            &[&[Network::Wojakcoin], &[Network::Regtest], &[Network::Testnet, Network::Signet]];
    ...
    fn p2tr_from_untweaked() {
        ...
        let address = Address::p2tr(&secp, internal_key, None, Network::Wojakcoin);
        assert_eq!(
            address.to_string(),
            "bc1p5cyxnuxmeuwuvkwfem96lqzszd02n6xdcjrs20cac6yqjjwudpxqkedrcr"
        );
    ...
    fn test_is_related_to_pubkey_p2tr() {
        ...
        let address = Address::p2tr_tweaked(tweaked_pubkey, Network::Wojakcoin);
    ...
    fn test_fail_address_from_script() {
        ...
        assert_eq!(Address::from_script(&bad_p2wpkh, Network::Wojakcoin), expected);
        assert_eq!(Address::from_script(&bad_p2wsh, Network::Wojakcoin), expected);
        assert_eq!(
            Address::from_script(&invalid_segwitv0_script, Network::Wojakcoin),
    ...
    fn test_matches_script_pubkey() {
        ...
        for addr in &addresses {
            let addr = Address::from_str(addr).unwrap().require_network(Network::Wojakcoin).unwrap();
            for another in &addresses {
                let another =
                    Address::from_str(another).unwrap().require_network(Network::Wojakcoin).unwrap();
