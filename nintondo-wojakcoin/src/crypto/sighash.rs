...
impl EcdsaSighashType {
    ...
    pub fn from_consensus(n: u32) -> EcdsaSighashType {
        ...
        // In pepecoin Core, the SignatureHash function will mask the (int32) value with
...
/// being relayed on today's pepecoin network.
...
impl<R: BorrowMut<Transaction>> SighashCache<R> {
    ...
    /// ```
    /// use nintondo_wojakcoin::{absolute, Transaction, Script};
    /// use nintondo_wojakcoin::sighash::{EcdsaSighashType, SighashCache};
    ///
...
mod tests {
    ...
    fn test_sighashes_keyspending() {
        // following test case has been taken from pepecoin Core test framework
    ...
    fn p2pkh_hex(pk: &str) -> ScriptBuf {
        ...
        Address::p2pkh(&pk, Network::Wojakcoin).script_pubkey()
    }
