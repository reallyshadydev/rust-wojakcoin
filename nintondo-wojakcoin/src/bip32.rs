...
mod tests {
    ...
    /// Derive along path and assert roundtrip (no fixed expected strings).
    fn test_path_roundtrip<C: secp256k1::Signing + secp256k1::Verification>(
        secp: &Secp256k1<C>,
        network: Network,
        seed: &[u8],
        path: DerivationPath,
        ...
        assert_eq!(ExtendedPrivKey::from_str(&sk.to_string()).unwrap(), sk);
        assert_eq!(ExtendedPubKey::from_str(&pk.to_string()).unwrap(), pk);
    }
    ...
    fn test_vector_1() {
        ...
        for path in [
            ...
            test_path_roundtrip(&secp, Wojakcoin, &seed, path.parse().unwrap());
        }
    }
