...
//! Wojakcoin network constants.
//!
//! This module provides various constants relating to the Wojakcoin network
...
//!
//! ```rust
//! use nintondo_wojakcoin::network::constants::Network;
//! use nintondo_wojakcoin::consensus::encode::serialize;
//!
//! let network = Network::Wojakcoin;
//! let bytes = serialize(&network.magic());
//!
//! assert_eq!(&bytes[..], &[0x6f, 0x8d, 0xa5, 0x79]);
//! ```
...
impl Network {
    ...
    ///
    /// ```rust
    /// use nintondo_wojakcoin::network::constants::{Network, Magic};
    /// use std::convert::TryFrom;
    ///
    /// assert_eq!(Ok(Network::Wojakcoin), Network::try_from(Magic::from_bytes([0x6f, 0x8d, 0xa5, 0x79])));
    ...
    ///
    /// let network = Network::Wojakcoin;
    /// assert_eq!(network.magic(), Magic::from_bytes([0x6f, 0x8d, 0xa5, 0x79]));
    /// ```
    ...
    /// ```rust
    /// use nintondo_wojakcoin::network::constants::Network;
    /// use nintondo_wojakcoin::blockdata::constants::ChainHash;
    ///
    /// let network = Network::Wojakcoin;
    /// assert_eq!(network.chain_hash(), ChainHash::WOJAKCOIN);
    /// ```
    ...
    /// use std::convert::TryFrom;
    ///
    /// assert_eq!(Ok(Network::Wojakcoin), Network::try_from(ChainHash::WOJAKCOIN));
    /// ```
...
impl TryFrom<ChainHash> for Network {
    ...
    fn try_from(chain_hash: ChainHash) -> Result<Self, Self::Error> {
        match chain_hash {
            ChainHash::WOJAKCOIN => Ok(Network::Wojakcoin),
...
impl Magic {
    /// Wojakcoin mainnet network magic bytes (from wojakcore chainparams).
    pub const WOJAKCOIN: Self = Self([0x6f, 0x8d, 0xa5, 0x79]);
    /// Wojakcoin testnet network magic bytes.
    pub const TESTNET: Self = Self([0x4d, 0xaa, 0x61, 0xf9]);
...
impl From<Network> for Magic {
    fn from(network: Network) -> Magic {
        match network {
            Network::Wojakcoin => Magic::WOJAKCOIN,
...
impl TryFrom<Magic> for Network {
    ...
    fn try_from(magic: Magic) -> Result<Self, Self::Error> {
        match magic {
            Magic::WOJAKCOIN => Ok(Network::Wojakcoin),
...
impl ServiceFlags {
    ...
    /// set by all pepecoin Core non pruned nodes, and is unset by SPV clients or other light
    /// clients.
    pub const NETWORK: ServiceFlags = ServiceFlags(1 << 0);

    /// GETUTXO means the node is capable of responding to the getutxo protocol request.  pepecoin
    /// Core does not support this but a patch set called pepecoin XT does.
    /// See BIP 64 for details on how this is implemented.
    pub const GETUTXO: ServiceFlags = ServiceFlags(1 << 1);

    /// BLOOM means the node is capable and willing to handle bloom-filtered connections.  pepecoin
...
mod tests {
    ...
    #[test]
    fn string_test() {
        assert_eq!(Network::Wojakcoin.to_string(), "wojakcoin");
        assert_eq!(Network::Testnet.to_string(), "testnet");
        assert_eq!(Network::Regtest.to_string(), "regtest");
        assert_eq!(Network::Signet.to_string(), "signet");

        assert_eq!("wojakcoin".parse::<Network>().unwrap(), Network::Wojakcoin);
