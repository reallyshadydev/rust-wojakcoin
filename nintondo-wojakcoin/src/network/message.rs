...
//! pepecoin network messages.
//!
//! This module defines the `NetworkMessage` and `RawNetworkMessage` types that
//! are used for (de)serializing pepecoin objects for transmission on the network.
//!
...
/// [pepecoin Wiki: Protocol Specification](https://en.bitcoin.it/wiki/Protocol_specification)
...
mod test {
    ...
    fn serialize_verack_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Wojakcoin), payload: NetworkMessage::Verack }),
                   vec![0x6f, 0x8d, 0xa5, 0x79, 0x76, 0x65, 0x72, 0x61,
    ...
    fn serialize_ping_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Wojakcoin), payload: NetworkMessage::Ping(100) }),
                   vec![0x6f, 0x8d, 0xa5, 0x79, 0x70, 0x69, 0x6e, 0x67,
    ...
    fn serialize_mempool_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Wojakcoin), payload: NetworkMessage::MemPool }),
                   vec![0x6f, 0x8d, 0xa5, 0x79, 0x6d, 0x65, 0x6d, 0x70,
    ...
    fn serialize_getaddr_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Wojakcoin), payload: NetworkMessage::GetAddr }),
                   vec![0x6f, 0x8d, 0xa5, 0x79, 0x67, 0x65, 0x74, 0x61,
    ...
    fn deserialize_getaddr_test() {
        #[rustfmt::skip]
        let msg = deserialize(&[
            0x6f, 0x8d, 0xa5, 0x79, 0x67, 0x65, 0x74, 0x61,
            0x64, 0x64, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x5d, 0xf6, 0xe0, 0xe2
        ]);
        let preimage = RawNetworkMessage {
            magic: Magic::from(Network::Wojakcoin),
    ...
    fn deserialize_version_test() {
        ...
        let msg = deserialize::<RawNetworkMessage>(&[
            0x6f, 0x8d, 0xa5, 0x79, 0x76, 0x65, 0x72, 0x73,
        ...
        assert!(msg.is_ok());
        let msg = msg.unwrap();
        assert_eq!(msg.magic, Magic::from(Network::Wojakcoin));
