...
impl OutPoint {
    ...
    ///
    /// ```rust
    /// use nintondo_wojakcoin::constants::genesis_block;
    /// use nintondo_wojakcoin::network::constants::Network;
    ///
    /// let block = genesis_block(Network::Wojakcoin);
...
/// pepecoin transaction input sequence number.
///
...
impl Sequence {
    ...
    /// is equivalent and improves readability for those not steeped in pepecoin folklore.
    ///
    /// ## Historical note
    ///
    /// The term 'final' is an archaic pepecoin term, it may have come about because the sequence
    /// number in the original pepecoin code was intended to be incremented in order to replace a
    /// transaction, so once the sequence number got to `u64::MAX` it could no longer be increased,
    /// hence it was 'final'.
    ///
    ///
    /// Some other references to the term:
    /// - `CTxIn::SEQUENCE_FINAL` in the pepecoin Core code.
    ...
    /// to 512 seconds.
    ///
    /// Encoding finer granularity of time for relative lock-times is not supported in pepecoin
    #[inline]
...
impl<E> EncodeSigningDataResult<E> {
    ...
    ///
    /// ```rust
    /// # use nintondo_wojakcoin::consensus::deserialize;
    /// # use nintondo_wojakcoin::sighash::{LegacySighash, SighashCache};
    /// # use nintondo_wojakcoin::Transaction;
    /// # use bitcoin_hashes::{Hash, hex::FromHex};
    /// # let mut writer = LegacySighash::engine();
    /// # let input_index = 0;
    /// # let script_pubkey = nintondo_wojakcoin::ScriptBuf::new();
...
/// pepecoin transaction.
///
/// An authenticated movement of coins.
///
/// See [pepecoin Wiki: Transaction][wiki-transaction] for more information.
///
...
impl Transaction {
    ...
    /// is different to what is implemented in pepecoin Core. The computation should be the same for
...
mod tests {
    ...
    fn test_is_coinbase() {
        ...
        let genesis = constants::genesis_block(Network::Wojakcoin);
    ...
    fn test_transaction_verify() {
        ...
        match spending
            ...
            .unwrap()
        {
            script::Error::WojakcoinConsensus(_) => {}
