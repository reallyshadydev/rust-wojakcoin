...
/// Mainnet (Wojakcoin) pubkey address prefix. Addresses start with 'W'.
pub const PUBKEY_ADDRESS_PREFIX_MAIN: u8 = 0x49;
/// Mainnet (Wojakcoin) script address prefix.
pub const SCRIPT_ADDRESS_PREFIX_MAIN: u8 = 0x05;
/// Test (testnet, signet, regtest) pubkey address prefix.
pub const PUBKEY_ADDRESS_PREFIX_TEST: u8 = 0x6f;
...
/// Constructs and returns the coinbase (and only) transaction of the Wojakcoin genesis block.
fn wojakcoin_genesis_tx() -> Transaction {
    // Base
    ...
    let in_script = script::Builder::new()
        ...
        .push_slice(b"382017 Price Phillip Retires")
        .into_script();
    ret.input.push(TxIn {
    ...
    // Outputs - Wojakcoin genesis from chainparams.cpp
    ...
    ret.output.push(TxOut { value: 100 * COIN_VALUE, script_pubkey: out_script });
...
pub fn genesis_block(network: Network) -> Block {
    let txdata = vec![wojakcoin_genesis_tx()];
    let hash: sha256d::Hash = txdata[0].txid().into();
    let merkle_root = hash.into();
    match network {
        Network::Wojakcoin => Block {
            header: block::Header {
...
impl ChainHash {
    // Wojakcoin mainnet genesis from wojakcore (display order: 000000004536a4f8...)
    /// `ChainHash` for Wojakcoin mainnet.
    pub const WOJAKCOIN: Self = Self([
        0x85, 0xf0, 0x7b, 0x42, 0xc0, 0x30, 0xf0, 0xa2, 0x83, 0x63, 0xba, 0xd3, 0x9f, 0x8d, 0x5f,
        0x82, 0xf9, 0xa9, 0x1c, 0x00, 0xf0, 0x88, 0x9d, 0xfa, 0xf8, 0xa4, 0x36, 0x45, 0x00, 0x00,
        0x00, 0x00,
    ]);
    /// `ChainHash` for testnet.
    pub const TESTNET: Self = Self([
        122, 26, 76, 141, 156, 47, 154, 92, 79, 38, 189, 141, 190, 218, 218, 114, 230, 246, 202,
        226, 181, 148, 205, 168, 253, 171, 119, 19, 113, 195, 150, 94,
    ]);
    /// `ChainHash` for signet.
    pub const SIGNET: Self = Self([
        222, 228, 251, 54, 69, 93, 157, 132, 27, 164, 147, 210, 69, 207, 84, 219, 108, 50, 137,
        41, 34, 137, 10, 77, 29, 201, 222, 224, 67, 48, 139, 221,
    ]);
    /// `ChainHash` for regtest.
    pub const REGTEST: Self = Self([
        99, 71, 172, 240, 140, 186, 110, 183, 51, 162, 14, 186, 144, 213, 40, 142, 218, 76, 92, 141,
        23, 134, 210, 0, 137, 222, 220, 63, 176, 31, 137, 21,
    ]);

    /// Returns the hash of the `network` genesis block for use as a chain hash.
    pub const fn using_genesis_block(network: Network) -> Self {
        let hashes = [Self::WOJAKCOIN, Self::TESTNET, Self::SIGNET, Self::REGTEST];
...
mod test {
    ...
    #[test]
    fn wojakcoin_genesis_first_transaction() {
        let gen = wojakcoin_genesis_tx();
    ...
    #[test]
    fn wojakcoin_genesis_full_block() {
        let gen = genesis_block(Network::Wojakcoin);

        assert_eq!(gen.header.version, block::Version::ONE);
        assert_eq!(gen.header.prev_blockhash, Hash::all_zeros());
        assert_eq!(gen.header.time, 1501724714);
        assert_eq!(gen.header.bits, CompactTarget::from_consensus(0x1d00ffff));
        assert_eq!(gen.header.nonce, 1252099851);
        assert_eq!(
    ...
    chain_hash_genesis_block! {
        mainnet_chain_hash_genesis_block, Network::Wojakcoin;
    ...
    fn mainnet_chain_hash_test_vector() {
        let got = ChainHash::using_genesis_block(Network::Wojakcoin).to_string();
        let want = "85f07b42c030f0a28363bad39f8d5f82f9a91c00f0889dfaf8a4364500000000";
        assert_eq!(got, want);
    }
}
