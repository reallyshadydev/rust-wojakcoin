...
//!
//! <details>
//! <summary>What is pepecoin script</summary>
//!
//! Scripts define pepecoin's digital signature scheme: a signature is formed
...
//! the script leaves `TRUE` on the stack after being evaluated. pepecoin's
...
//!
//! See [pepecoin Wiki: Script][wiki-script] for more information.
//!
...
//! and forcing them to be larger would waste memory and, in case of pepecoin script, even some
...
/// more than 4 bytes, this is in line with pepecoin Core (see [`CScriptNum::serialize`]).
///
...
///
/// This code is based on the `CScriptNum` constructor in pepecoin Core (see `script.h`).
pub fn read_scriptint(v: &[u8]) -> Result<i64, Error> {
    ...
    // Comment and code copied from pepecoin Core:
...
pub enum Error {
    ...
    pepecoinConsensus(bitcoinconsensus::Error),
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            #[cfg(feature = "bitcoinconsensus")]
            Error::WojakcoinConsensus(ref e) => {
...
impl std::error::Error for Error {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        ...
        match *self {
            ...
            | Serialization => None,
            #[cfg(feature = "bitcoinconsensus")]
            pepecoinConsensus(ref e) => Some(bitcoinconsensus_hack::wrap_error(e)),
        }
    }
}
