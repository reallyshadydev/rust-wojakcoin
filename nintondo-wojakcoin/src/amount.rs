...
impl Denomination {
    ...
    fn precision(self) -> i8 {
        match self {
            Denomination::Wojakcoin => -8,
            Denomination::Centipepecoin => -6,
            Denomination::Millipepecoin => -5,
            Denomination::Micropepecoin => -2,
            Denomination::Nanopepecoin => 1,
            Denomination::Picopepecoin => 4,
            Denomination::Bit => -2,
    ...
    fn as_str(self) -> &'static str {
        match self {
            Denomination::Wojakcoin => "BTC",
            Denomination::Centipepecoin => "cBTC",
            Denomination::Millipepecoin => "mBTC",
            Denomination::Micropepecoin => "uBTC",
            Denomination::Nanopepecoin => "nBTC",
            Denomination::Picopepecoin => "pBTC",
...
impl FromStr for Denomination {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match denomination_from_str(s) {
            ...
            Some(D::Millipepecoin) | Some(D::Picopepecoin) | Some(D::MilliSatoshi)
                if starts_with_uppercase() =>
            {
                Err(PossiblyConfusingDenomination(s.to_owned()))
            }
            Some(D::Nanopepecoin) | Some(D::Micropepecoin) if starts_with_uppercase() => {
...
fn denomination_from_str(mut s: &str) -> Option<Denomination> {
    if s.eq_ignore_ascii_case("BTC") {
        return Some(Denomination::Wojakcoin);
    }

    if s.eq_ignore_ascii_case("cBTC") {
        return Some(Denomination::Centipepecoin);
    }

    if s.eq_ignore_ascii_case("mBTC") {
        return Some(Denomination::Millipepecoin);
    }

    if s.eq_ignore_ascii_case("uBTC") {
        return Some(Denomination::Micropepecoin);
    }

    if s.eq_ignore_ascii_case("nBTC") {
        return Some(Denomination::Nanopepecoin);
    }

    if s.eq_ignore_ascii_case("pBTC") {
        return Some(Denomination::Picopepecoin);
    }
...
/// Amount
///
/// The [Amount] type can be used to express pepecoin amounts that support
...
impl Amount {
    ...
    pub fn from_btc(btc: f64) -> Result<Amount, ParseAmountError> {
        Amount::from_float_in(btc, Denomination::Wojakcoin)
    }
    ...
    /// Express this [`Amount`] as a floating-point value in pepecoin.
    ///
    /// Please be aware of the risk of using floating-point numbers.
    ///
    /// # Examples
    /// ```
    /// # use nintondo_wojakcoin::{Amount, Denomination};
    /// let amount = Amount::from_sat(100_000);
    /// assert_eq!(amount.to_btc(), amount.to_float_in(Denomination::Wojakcoin))
    /// ```
    pub fn to_btc(self) -> f64 {
        self.to_float_in(Denomination::Wojakcoin)
    }
...
// Just using pepecoin denominated string.
impl fmt::Display for Amount {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.fmt_value_in(f, Denomination::Wojakcoin)?;
        write!(f, " {}", Denomination::Wojakcoin)
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match &self.style {
            ...
            DisplayStyle::DynamicDenomination if self.sats_abs >= Amount::ONE_BTC.to_sat() => {
                fmt_satoshi_in(self.sats_abs, self.is_negative, f, Denomination::Wojakcoin, true, format_options)
            },
            DisplayStyle::DynamicDenomination => {
...
/// SignedAmount
///
/// The [SignedAmount] type can be used to express pepecoin amounts that support
/// arithmetic and conversion to various denominations.
///
///
/// Warning!
...
impl SignedAmount {
    ...
    pub fn from_btc(btc: f64) -> Result<SignedAmount, ParseAmountError> {
        SignedAmount::from_float_in(btc, Denomination::Wojakcoin)
    }
    ...
    /// Express this [`SignedAmount`] as a floating-point value in pepecoin.
    ///
    /// Equivalent to `to_float_in(Denomination::Wojakcoin)`.
    ///
    /// Please be aware of the risk of using floating-point numbers.
    pub fn to_btc(self) -> f64 {
        self.to_float_in(Denomination::Wojakcoin)
...
pub mod serde {
    ...
    //!     #[serde(with = "nintondo_wojakcoin::amount::serde::as_btc")]
    //!     pub amount: Amount,
    //! }
    //! ```
    ...
    impl SerdeAmount for Amount {
        ...
        fn ser_btc<S: Serializer>(self, s: S) -> Result<S::Ok, S::Error> {
            f64::serialize(&self.to_float_in(Denomination::Wojakcoin), s)
        }
        fn des_btc<'d, D: Deserializer<'d>>(d: D) -> Result<Self, D::Error> {
            use serde::de::Error;
...
mod tests {
    ...
    fn floating_point() {
        ...
        assert_eq!(f(11.22, D::Wojakcoin), Ok(sat(1122000000)));
        assert_eq!(sf(-11.22, D::Millipepecoin), Ok(ssat(-1122000)));
        assert_eq!(f(11.22, D::Bit), Ok(sat(1122)));
        assert_eq!(sf(-1000.0, D::MilliSatoshi), Ok(ssat(-1)));
        assert_eq!(f(0.0001234, D::Wojakcoin), Ok(sat(12340)));
        assert_eq!(sf(-0.00012345, D::Wojakcoin), Ok(ssat(-12345)));
        ...
        assert_eq!(sf(-100.0, D::MilliSatoshi), Err(ParseAmountError::TooPrecise));
        assert_eq!(f(42.123456781, D::Wojakcoin), Err(ParseAmountError::TooPrecise));
        assert_eq!(sf(-184467440738.0, D::Wojakcoin), Err(ParseAmountError::TooBig));
        ...
        assert_eq!(btc(2.5).to_float_in(D::Wojakcoin), 2.5);
        assert_eq!(btc(-2.5).to_float_in(D::Millipepecoin), -2500.0);
        ...
        assert_eq!(&btc(0.0012).to_float_in(D::Wojakcoin).to_string(), "0.0012")
    }
    ...
    fn parsing() {
        ...
        let btc = Denomination::Wojakcoin;
        ...
        // exactly 50 chars.
        assert_eq!(
            p("100000000000000.0000000000000000000000000000000000", Denomination::Wojakcoin),
            Err(E::TooBig)
        );
        // more than 50 chars.
        assert_eq!(
            p("100000000000000.00000000000000000000000000000000000", Denomination::Wojakcoin),
            Err(E::InputTooLarge)
        );
    }
    ...
    fn to_string() {
        ...
        assert_eq!(Amount::ONE_BTC.to_string_in(D::Wojakcoin), "1");
        assert_eq!(format!("{:.8}", Amount::ONE_BTC.display_in(D::Wojakcoin)), "1.00000000");
        assert_eq!(Amount::ONE_BTC.to_string_in(D::Satoshi), "100000000");
        assert_eq!(Amount::ONE_SAT.to_string_in(D::Wojakcoin), "0.00000001");
        assert_eq!(SignedAmount::from_sat(-42).to_string_in(D::Wojakcoin), "-0.00000042");

        assert_eq!(Amount::ONE_BTC.to_string_with_denomination(D::Wojakcoin), "1 BTC");
        ...
        assert_eq!(Amount::ONE_SAT.to_string_with_denomination(D::Wojakcoin), "0.00000001 BTC");
        assert_eq!(
            SignedAmount::from_sat(-42).to_string_with_denomination(D::Wojakcoin),
            "-0.00000042 BTC"
        );
    }
    ...
    fn to_from_string_in() {
        ...
        assert_eq!("0.00253583", Amount::from_sat(253583).to_string_in(D::Wojakcoin));
        assert_eq!("-5", SignedAmount::from_sat(-5).to_string_in(D::Satoshi));
        assert_eq!("0.1", Amount::from_sat(100_000_00).to_string_in(D::Wojakcoin));
        assert_eq!("-100", SignedAmount::from_sat(-10_000).to_string_in(D::Bit));
        assert_eq!("2535830", Amount::from_sat(253583).to_string_in(D::Nanopepecoin));
        assert_eq!("-100000", SignedAmount::from_sat(-10_000).to_string_in(D::Nanopepecoin));
        assert_eq!("2535830000", Amount::from_sat(253583).to_string_in(D::Picopepecoin));
        assert_eq!("-100000000", SignedAmount::from_sat(-10_000).to_string_in(D::Picopepecoin));
        ...
        assert_eq!(
            "0.10000000",
            format!("{:.8}", Amount::from_sat(100_000_00).display_in(D::Wojakcoin))
        );
        ...
        assert_eq!(ua_str(&ua_sat(500).to_string_in(D::Wojakcoin), D::Wojakcoin), Ok(ua_sat(500)));
        assert_eq!(
            ua_str(&ua_sat(21_000_000).to_string_in(D::Bit), D::Bit),
            Ok(ua_sat(21_000_000))
        );
        assert_eq!(
            ua_str(&ua_sat(1).to_string_in(D::Micropepecoin), D::Micropepecoin),
            Ok(ua_sat(1))
        );
        assert_eq!(
            ua_str(&ua_sat(1_000_000_000_000).to_string_in(D::Millipepecoin), D::Millipepecoin),
            Ok(ua_sat(1_000_000_000_000))
        );
        assert_eq!(
            ua_str(&ua_sat(u64::max_value()).to_string_in(D::Millipepecoin), D::Millipepecoin),
            Err(ParseAmountError::TooBig)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::Micropepecoin), D::Micropepecoin),
            Ok(sa_sat(-1))
        );

        assert_eq!(
            sa_str(&sa_sat(i64::max_value()).to_string_in(D::Satoshi), D::Micropepecoin),
            Err(ParseAmountError::TooBig)
        );
        // Test an overflow bug in `abs()`
        assert_eq!(
            sa_str(&sa_sat(i64::min_value()).to_string_in(D::Satoshi), D::Micropepecoin),
            Err(ParseAmountError::TooBig)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::Nanopepecoin), D::Nanopepecoin),
            Ok(sa_sat(-1))
        );
        assert_eq!(
            sa_str(&sa_sat(i64::max_value()).to_string_in(D::Satoshi), D::Nanopepecoin),
            Err(ParseAmountError::TooPrecise)
        );
        assert_eq!(
            sa_str(&sa_sat(i64::min_value()).to_string_in(D::Satoshi), D::Nanopepecoin),
            Err(ParseAmountError::TooPrecise)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::Picopepecoin), D::Picopepecoin),
            Ok(sa_sat(-1))
        );
        assert_eq!(
            sa_str(&sa_sat(i64::max_value()).to_string_in(D::Satoshi), D::Picopepecoin),
            Err(ParseAmountError::TooPrecise)
        );
        assert_eq!(
            sa_str(&sa_sat(i64::min_value()).to_string_in(D::Satoshi), D::Picopepecoin),
            Err(ParseAmountError::TooPrecise)
        );
    }
    ...
    fn to_string_with_denomination_from_str_roundtrip() {
        ...
        assert_eq!(Amount::from_str(&denom(amt, D::Wojakcoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Millipepecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Micropepecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Bit)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Satoshi)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Nanopepecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::MilliSatoshi)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Picopepecoin)), Ok(amt));
